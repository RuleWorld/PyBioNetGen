...
def setupDatabase(databaseName):
    ...
    cursor = connection.cursor()
    cursor.execute(
        """CREATE TABLE biomodels(file UNIQUE, organismID INT, FOREIGN KEY(organismID) REFERENCES annotation(ROWID))"""
    )
    cursor.execute(
        """CREATE TABLE moleculeNames(fileId INT,name, FOREIGN KEY(fileID) REFERENCES biomodels(file))"""
    )
    cursor.execute(
        """CREATE TABLE annotation(annotationURI UNIQUE ON CONFLICT IGNORE,annotationName)"""
    )
    cursor.execute(
        """CREATE TABLE identifier(annotationID INT, qualifier, speciesID INT, FOREIGN KEY(speciesID) \
                   REFERENCES moleculeName(ROWID), FOREIGN KEY(annotationID) references annotation(ROWID))"""
    )
    cursor.execute(
        """CREATE TABLE bond(fileName INT, moleculeID1 INT, moleculeID2 INT, FOREIGN KEY(fileName) REFERENCES biomodels(ROWID), \
...
def extractBasicAnnotations(fileName, userDefinitions=None):
    annotations = utils.annotationExtractor.AnnotationExtractor(
        fileName, userDefinitions
    )
    elementalMolecules = [x for x in annotations.sct if annotations.sct[x] == []]
    speciesAnnotations = {
        x: annotations.getAnnotationSystem()[x] for x in elementalMolecules
    }
...
def populateDatabaseFromFile(fileName, databaseName, userDefinitions=None):
    ...
    basicModelAnnotations, generalModelAnnotations = extractBasicAnnotations(
        fileName, userDefinitions
    )
        ...
        for annotationType in basicModelAnnotations[molecule]:
            for annotation in basicModelAnnotations[molecule][annotationType]:
                if annotation not in annotationIDs:
                    annotationName = utils.annotationResolver.resolveAnnotation(
                        annotation
                    )
    ...
    for molecule in basicModelAnnotations:
        for annotationType in basicModelAnnotations[molecule]:
            for annotation in basicModelAnnotations[molecule][annotationType]:
                moleculeAnnotations.append(
                    [annotationIDs[annotation], moleculeIDs[molecule], annotationType]
                )
    cursor.executemany(
        "insert into identifier(annotationID,speciesID, qualifier) values (?, ?, ?)",
        moleculeAnnotations,
    )
    connection.commit()
    connection.close()
...
def defineConsole():
    ...
    parser.add_argument(
        "-c", "--create", action="store_true", help="Create database tables"
    )
    parser.add_argument(
        "-d", "--database", type=str, help="database to modify", required=True
    )
    ...
    parser.add_argument(
        ...
        "--user_conventions",
        type=str,
        help="Use user convention definitions for SCT calculation",
    )
    parser.add_argument(
        "-q", "--query", type=str, help="Query a database for its common namespace"
    )
    parser.add_argument(
        "-s",
        "--specific_query",
        type=str,
        help="search for models with a given molecule",
    )
    parser.add_argument(
        "-r",
        "--directory",
        type=str,
        help='Add SBML models in directory "directory" to database',
    )
    ...
    if namespace.query:
        ...
        if namespace.input_file:
            ...
            populateDatabaseFromFile(
                namespace.input_file, namespace.database, userDefinitions
            )
