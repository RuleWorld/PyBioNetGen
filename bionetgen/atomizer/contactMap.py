...
def simpleGraph(graph, species, observableList, prefix="", superNode={}):
    ...
    for singleSpecies in species:
        ...
        for component in singleSpecies.components:
            graph.add_node(
                "{0}({1})".format(name, component.name), size=0.5, color="teal"
            )
            edgeList.append(
                ["{0}".format(name), "{0}({1})".format(name, component.name), 10]
            )
    ...
    for cidx, cluster in enumerate(linkArray):
                    ...
                    for species in [
                        ...
                        try:
                            ...
                            if isinstance(speciesEquivalence[idx][species], str):
                                tmpEdgeList.append(
                                    "{0}_{1}".format(
                                        idx, speciesEquivalence[idx][species][0:-2]
                                    )
                                )
                                ...
                                tmpEdgeList.append(
                                    "{0}_{1}".format(
                                        idx,
                                        speciesEquivalence[idx][species]
                                        .molecules[0]
                                        .name,
                                    )
                                )
        ...
        for x in cluster:
            ...
            try:
                ...
                continue
            nodeDict[number] = simpleGraph(
                graph, species, observableList, number, superNodeDict
            )
