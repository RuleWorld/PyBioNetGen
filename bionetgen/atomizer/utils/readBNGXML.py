...
def parseRule(rule, parameterDict):
    ...
    if rateConstants == None:
        ...
        rateConstants = tmp
    rateConstantsValue = (
        parameterDict[rateConstants]
        if rateConstants in parameterDict
        else rateConstants
    )
    # rule = st.Rule()
    label = rule.get("name")
...
def parseMolecules(molecules):
    ...
    components = molecules.find(
        ".//{http://www.sbml.org/sbml/level3}ListOfComponentTypes"
    )
    if components != None:
...
def parseFullXML(xmlFile):
    ...
    for species in seedspecies:
        ...
        seedSpeciesList.append(
            {
                "concentration": concentration,
                "structure": createSpecies(species)[0],
...
def createBNGLFromDescription(namespace):
    ...
    for seedspecies in namespace["seedspecies"]:
        if seedspecies["concentration"] not in [0, "0", "0.0"]:
            bnglString.write(
                "\t{0} {1}\n".format(
                    seedspecies["structure"], seedspecies["concentration"]
                )
            )
    ...
    for observables in namespace["observables"]:
        bnglString.write(
            "\t{0} {1} {2}\n".format(
                observables[1],
                observables[0],
                ", ".join([str(x) for x in observables[2]]),
            )
        )
