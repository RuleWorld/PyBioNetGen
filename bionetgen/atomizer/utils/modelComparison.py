...
def defineConsole():
    ...
    parser.add_argument('-f2', '--filename2', type=str, help='comparison file', required=True)
    return parser
    ...
    for molecule in moleculeSet1:
        for molecule2 in moleculeSet2:
            if molecule.name == molecule2.name:
                ...
                scoreDict['componentSetSize'].extend([x.name for x in molecule.components if len(x.states) > 0])
                for component in [x for x in molecule.components if len(x.states) > 0]:
                    for component2 in [x for x in molecule2.components if len(x.states) > 0]:
...
def evaluateProcessSimilarity(bnglNamespace, bnglNamespace2):
    stdDictionary1 = stateTransitionDiagram.getContextRequirementsFromNamespace(bnglNamespace)
    stdDictionary2 = stateTransitionDiagram.getContextRequirementsFromNamespace(bnglNamespace2)
    intersectingNamespace = [x.name for x in bnglNamespace['molecules'] if x.name in [y.name for y in bnglNamespace2['molecules']]]
    ...
    for element in [x for x in stdDictionary1[0] if x in intersectingNamespace]:
        for state in stdDictionary1[0][element]:
            intersectionSpace[element].add(frozenset(x for x in state if x[0] in lowerIntersectionNamespace))

    for element in intersectionSpace:
        scoreDict[element]['file1'] = len(intersectionSpace[element])
        scoreDict[element]['totalSpace'] = 2 ** len(next(iter(intersectionSpace[element])))
        print element, scoreDict[element]['totalSpace'], next(iter(intersectionSpace[element]))
