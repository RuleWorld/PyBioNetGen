...
def componentAnalysis(directory):
    ...
    for model in moleculeTypesArray:
        ...
        modelComponentDict[model[-2]] = {'bindingComponents':np.mean(bindingComponentCount), 'stateComponents':np.mean(modificationComponentCount), 'componentCount': np.mean(modelComponentCount)}
