...
def createNode(graph, name, graphicsDict, labelGraphicsDict, isGroup, gid):
    ...
    if isGroup:
        if gid:
            graph.add_node(name, graphics=graphicsDict, LabelGraphics=labelGraphicsDict, isGroup=isGroup, gid=gid, id=idNumber)
        else:
...
def createPDLabelNode(graph, molecule, nodeList):
    globalLabelDict = {}
    for node in nodeList[molecule]:
        localNodeName = '{0}_{1}'.format(molecule, '/'.join([x[0] for x in node if x[1]]))
        labelDict = []
        ...
        createNode(graph, localNodeName, {'type': "roundrectangle", 'fill': '#FFFFFF', 'w':270, 'h':140}, {}, 0, graph.node[molecule]['id'])
...
def createPDGraphNode(graph, molecule, nodeList):
    for node in nodeList[molecule]:
        localNodeName = '{0}_{1}'.format(molecule, '/'.join([x[0] for x in node if x[1]]))
        createNode(graph, localNodeName, {'type': "roundrectangle", 'fill':'#FFFFFF'}, {'text': ' '}, 1, graph.node[molecule]['id'])
        ...
        for bit in node:
            if bit[1]:
                nodecolor = '#FF88FF'
                createNode(graph, '{0}_{1}'.format(localNodeid, bit[0]), {'type': "roundrectangle", 'fill': nodecolor},
                           {'text': bit[0]}, 0, localNodeid)
        ...
        for idx in range(0, len(subNodeId)-1):
            graph.add_edge(subNodeId[idx], subNodeId[idx+1], graphics={'fill': '#CCCCFF', 'style': 'dashed'})
...
def createPDEdge(graph, molecule, edge, edgeList):
    ...
    if ('{0}_{1}'.format(molecule, '/'.join(nodeId1)), '{0}_{1}'.format(molecule,  '/'.join(nodeId0))) not in graph.edges():
        ...
        processNodeName = '{0}_{1}_{2}_edge'.format(molecule, '/'.join(nodeId0), '/'.join(nodeId1))
