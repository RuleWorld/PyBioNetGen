...
sub findMap
{
  ...
  # if NOT syndel model we need to deal with symmtery
  # if it is, skip the symmtery calculation
  #
  # First let's get the number of operations in each type 
  my $ean = @{ $rr->EdgeAdd };
  my $edn = @{ $rr->EdgeDel };
  my $man = @{ $rr->MolAdd };
  my $mdn = @{ $rr->MolDel };
  my $scn = @{ $rr->CompStateChange };
  my $ccn = @{ $rr->ChangeCompartment };
  ...
  # printf "mol del:     %d\n", $mdn;
  # printf "comp state:  %d\n", $scn;
  # printf "change comp: %d\n", $ccn;
  #
  # Check to see if we have anything but mol add/del
  # TODO: We probably should raise an error if mol add/del is also 0
  if ( $ean > 0 | $edn > 0 | $scn > 0 | $ccn > 0 ) {
  ...
  {
    # We have a syndel reaction so we don't need to deal with 
    # symmetry, just set to 1
	  $rr->MultScale(1);
  }
