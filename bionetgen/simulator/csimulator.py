...
class CSimulator:
    ...
    def simulate(self, t_start=0, t_end=100, n_steps=100):
        ...
        self.result = self.return_type.from_address( self.lib.simulate(ntpts, ctypes.c_void_p(timepoints.ctypes.data), 
                                                     self.num_spec_init, ctypes.c_void_p(self.species_init.ctypes.data), 
                                                     self.num_params, ctypes.c_void_p(self.parameters.ctypes.data) ) )
        
        obs_names = ctypes.cast(self.result.obs_names, ctypes.POINTER(ctypes.c_char*self.result.obs_name_len))[0].value.decode()
        obs_names = obs_names.split(":")[:-1]
        
        spcs_names = ctypes.cast(self.result.spcs_names, ctypes.POINTER(ctypes.c_char*self.result.spcs_name_len))[0].value.decode()
        spcs_names = spcs_names.split(":")[:-1]
        
        buffer_as_ctypes_arr_obs = ctypes.cast(self.result.observables, ctypes.POINTER(ctypes.c_double*ntpts*self.result.n_observables))[0]
        ...
        buffer_as_ctypes_arr_spc = ctypes.cast(self.result.species, ctypes.POINTER(ctypes.c_double*ntpts*self.result.n_species))[0]
...
class BNGSim:
    def __init__(self, model, generate_network=False):
        ...
        subprocess.call(["gcc", "-fPIC", "-I/home/boltzmann/apps/cvode-2.6.0/cvode_lib/include/", "-c", f"{c_file}"])
        subprocess.call(["gcc", f"{obj_file}", "--shared", "-fPIC", 
                    "-L/home/boltzmann/apps/cvode-2.6.0/cvode_lib/lib/",
                     "-lsundials_cvode", "-lsundials_nvecserial", "-o", f"{lib_file}"])
        # use CSimulator under the hood
        n_param = len(list(filter(lambda x: not x.startswith("_"), list(self.model.parameters))))
        self.csim = CSimulator(os.path.abspath(lib_file), num_params=n_param, num_spec_init=len(self.model.species))
