...
def ato_write_gml(graph, fileName, labelGraphics):
    ...
    for node in graph.nodes:
        ...
        writeDict(gml, "node", node, graph.nodes[node], "\t", labelGraphics)
...
def createPDGraphNode(graph, molecule, nodeList):
    for node in nodeList[molecule]:
        ...
        localNodeid = graph.nodes[localNodeName]["id"]
        subNodeId = []
        for bit in node:
            if bit[1]:
                nodecolor = "#FF88FF"
...
if __name__ == "__main__":
    ...
    nx.write_gml(graph, inputFile.replace(".xml", "") + "_std.gml")
